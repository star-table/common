...
func TestProxy_ConsumePushMessage(t *testing.T) {
	...
	proxy := Proxy{}
	_ = proxy.ConsumeMessage("test", "", func(msg *mq.MqMessageExt) errors.SystemErrorInfo {
...
func TestProxy_SendMessage(t *testing.T) {
	...
	_, err := proxy.PushMessage(&mq.MqMessage{
		Topic:     "test",
		Partition: 0,
		Body:      "hello",
	})
...
func TestProxy_ConsumeGroupMessage(t *testing.T) {
	...
	proxy.ConsumeMessage("testabc", "test1", func(msg *mq.MqMessageExt) errors.SystemErrorInfo {
		log.Infof("msg offset: %d, partition: %d,  value: %s", msg.Offset, msg.Partition, string(msg.Body))
		return nil
	})
